...
public class StatsController {
    ...
    public List<ViewStats> getStats(
            @RequestParam @DateTimeFormat(pattern = CommonUtils.DT_FORMAT) LocalDateTime start,
            @RequestParam @DateTimeFormat(pattern = CommonUtils.DT_FORMAT) LocalDateTime end,
            @RequestParam(required = false) List<String> uris,
            @RequestParam(required = false, defaultValue = "false") Boolean unique) {

        if (start.isAfter(end)) {
            throw new IllegalArgumentException("Дата начала мероприятия не может быть после даты его окончания");
        }
